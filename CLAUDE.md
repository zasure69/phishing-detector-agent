...
| **MiniMax M2.5** | `minimax/minimax-m2.5` | Cross-validation kết quả Qwen+Gemma để giảm false positive (model này **KHÔNG đa phương thức** — không đọc được ảnh) |
| **Vision (Gemma/Qwen)** | `google/gemma-4-31b-it` | Đọc ảnh chụp màn hình (OCR + dấu hiệu thị giác). MiniMax không hỗ trợ ảnh nên dùng Gemma. |
...
| `agent/vision.py` | Đọc ảnh chụp màn hình (Gemma) → text + URL hiển thị + dấu hiệu thị giác |
...
- [x] Phân tích **ảnh chụp màn hình** (Gemma vision) + dán ảnh (Ctrl+V) — cho SMS/Zalo/Messenger. Có cảnh báo "độ tin cậy thấp hơn .eml" (ảnh không thấy link thật/header)
